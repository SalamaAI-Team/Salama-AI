...
export function ChatWindow(props: {
  ...
  return (
    <div className={`flex flex-col items-center p-4 md:p-8 rounded grow  text-bodydark1 dark:bg-boxdark-2 dark:text-bodydark1 overflow-hidden ${(messages.length > 0 ? "border" : "")}`}>
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        ...
        <div className="flex w-full mt-4">
          <input
            className="grow mr-8 p-4 rounded dark:bg-boxdark-2 dark:text-bodydark1"
            value={input}
            placeholder={placeholder ?? "I am Non Communicable Diseases Expert"}
          ...
          <button type="submit" className="shrink-0 px-8 py-4 bg-sky-600 rounded w-28  ">
            <div role="status" className={`${(chatEndpointIsLoading || intermediateStepsLoading) ? "" : "hidden"} flex justify-center`}>
              <svg aria-hidden="true" className="w-6 h-6 text-boxdark-2 animate-spin dark:text-white fill-sky-800" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
