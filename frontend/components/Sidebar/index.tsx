...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  useEffect(() => {
    const getSessionAndUser = async () => {
      try {
        ...
        if (user) {
          setUserRole(user?.role || "John Doe");
        }
      } catch (error) {
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden bg-blue-500 duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpen ? "translate-x-0" : "-translate-x-full"
      }`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-between gap-1 px-4 py-3.5 lg:py-3.5">
        <Link href="/dashboard">
          <Image
            width={100}
            height={32}
